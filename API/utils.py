...
def get_session_workspace(session_id: str) -> str:
    ...
    继承原始项目设计，与get_thread_workspace功能相同但命名更清晰。
    """
    if not session_id:
        session_id = "default"
...
def build_download_url(rel_path: str) -> str:
    ...
    Args:
        rel_path: 相对于workspace的路径，如 "session_id/filename.csv"
        
    Returns:
        完整的HTTP文件服务器URL，如 "http://localhost:8100/session_id/filename.csv"
    """
    try:
...
def prepare_vllm_messages(
    ...
    - Optionally prepend system prompt from LLM Manager configuration
    ...
    if system_prompt and system_prompt.strip():
        ...
        if not has_system:
            vllm_messages.append({"role": "system", "content": system_prompt.strip()})
    ...
    if last_user_idx is not None:
        ...
        # 检查用户消息是否已包含 # Instruction 标记（避免重复添加）
        already_has_instruction = instruction_body.startswith("# Instruction")
        
        if workspace_file_info:
            if already_has_instruction:
                # 已有 # Instruction 标记，直接追加 # Data
            ...
            if not already_has_instruction:
                vllm_messages[last_user_idx]["content"] = f"# Instruction\n{instruction_body}"
            # 如果已有 # Instruction 标记且无 workspace 文件，保持原样
...
def generate_report_from_messages(
    ...
    try:
        ...
        # 构建包含thread_id前缀的完整路径
        url = build_download_url(f"{thread_id}/{rel_path}")
...
def render_file_block(
    ...
    for p in artifact_paths:
        try:
            ...
            rel = Path(p).name
        # 构建包含thread_id前缀的完整路径
        url = build_download_url(f"{thread_id}/{rel}")
        name = Path(p).name
        if generated_files_sink is not None :
