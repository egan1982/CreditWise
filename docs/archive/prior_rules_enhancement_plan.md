...
> **创建日期**: 2026-01-12  
> **最后更新**: 2026-04-17  
> **状态**: ✅ Phase 1 已实施 + 测试通过（2026-04-17，12/12 PASS）  
> **优先级**: P2  
> **开发评审**: Eng Review + Design Review 已完成（2026-04-16），4 项必须修订 + 3 项建议修订
...
| # | 问题 | 说明 | 建议修复 |
|---|------|------|---------|
| **E1** | **过度抽象（4 个新类）** | 方案设计了 PriorRuleParser + RuleValidator + ThresholdComparator + PriorRuleManager 四个独立类，触发复杂度阈值（9 文件 + 4 新类）。精简版只需 CSV→list[str] | 合并为 1 个 `PriorRuleParser` 类 + 1 个 `compare_thresholds()` 函数。删除 RuleValidator（嵌入 Parser）、删除 PriorRuleManager（不需要中间层） |
| **E2** | **prior_rules 类型断层（现有 Bug）** | 前端 textarea → 纯字符串 → executor 透传 → `run(prior_rules: list[str])`。`PriorRuleAnalyzer.fit()` 中 `for pr in prior_rules` 会逐字符迭代字符串 | executor.py 或 `run()` 入口加 `if isinstance(prior_rules, str): prior_rules = [r.strip() for r in prior_rules.split('\n') if r.strip()]` |
| **E5** | **eval() 安全隐患** | `_build_expression` L422 用 `eval(threshold)` 解析 between 的 `[min, max]`，用户上传 CSV 的 threshold 字段可能包含恶意代码 | 改为 `ast.literal_eval()` 或 `json.loads()` |
| **F3** | **文件路径注入** | `/prior-rules/parse` 端点直接写 `/tmp/{file.filename}`，file.filename 可包含 `../` | 使用 `tempfile.NamedTemporaryFile`，用完即删 |

#### 🟡 建议修订

| # | 建议 | 理由 |
|---|------|------|
| **E6** | **阈值对比限制在 UI 明确提示** | 仅支持简单规则（`feature op value`），组合规则无法解析，应在前端说明 |
| **D3** | **"从历史导入" Tab 暂不实现** | 方案正文几乎无设计，标记 Phase 2 |
| **工作量** | **29h → ~8-12h（1-1.5天）** | 精简后只需 1 个 Parser 类 + 1 个 API 端点 + 1 个前端组件 |

#### 🔴 关键失败模式

| # | 代码路径 | 失败场景 | 后果 | 严重性 |
|---|---------|---------|------|:------:|
| **F1** | `for pr in prior_rules`（收到字符串） | 逐字符迭代，生成垃圾结果 | 🔴 现有 Bug |
| **F2** | `eval(threshold)` | CSV 含恶意代码 → 任意代码执行 | 🔴 安全 |
| **F3** | `/tmp/{file.filename}` | 路径注入 → 文件覆盖/读取 | 🔴 安全 |

#### 📋 测试覆盖度

方案原有测试用例合理，但缺少 4 类场景：

| # | 缺失场景 | 重要性 |
|---|---------|:---:|
| **T1** | 空文件 / 空文本测试 | 🟡 |
| **T2** | 编码异常（GBK/BOM 等）测试 | 🟡 |
| **T3** | 大文件（1000+ 条规则）性能测试 | 🟡 |
| **T4** | prior_rules 类型断层的回归测试 | 🔴 |

#### 📊 Eng Review Summary

```
Step 0: Scope Challenge       — 触发复杂度阈值，建议缩减为 1 类 + 1 函数
Architecture Review           — 4 issues (E1 过度抽象, E2 类型断层, E5 eval, F3 路径注入)
Code Quality Review           — E6 阈值对比限制提示
Test Review                   — 缺 4 类场景 (T1-T4)
Performance Review            — 0 issues
Failure modes                 — 3 critical (F1 现有Bug, F2+F3 安全)
```
...
> **审查方法**: plan-design-review（信息架构/交互状态/用户旅程/设计系统 四维评审）

#### 📊 评分总览

| Pass | 维度 | 评分 | 说明 |
|------|------|:----:|------|
| 1 | 信息架构 | 8/10 | Tab 切换（手动/文件/历史）层级清晰 |
| 2 | 交互状态 | 7/10 | 缺上传中 loading 状态和大文件进度提示 |
| 3 | 用户旅程 | 8/10 | 手动→文件的渐进增强合理，模板下载降低门槛 |
| 4 | 设计系统 | 9/10 | 复用 shadcn/ui 组件 |
| **总分** | | **8/10** | |
...
| # | 建议 | 说明 |
|---|------|------|
| **D1** | **上传 loading 状态** | 补充文件上传中的 loading 态 + 大文件进度指示 |
| **D2** | **校验结果展示** | 校验失败时高亮问题行，成功时展示解析出的规则数量 |
| **D3** | **"从历史导入" Tab 延后** | 正文无设计，精简版暂隐藏此 Tab，Phase 2 再开放 |
...
经业务逻辑评估和行业惯例分析（参考：冯占鹏《Python金融风控策略实践》§4.3 Swap Set 分析、FICO Champion-Challenger 框架），策略诊断（新旧策略对比评估）在行业中是**独立于策略开发**的分析活动，其分析维度（人数/金额双口径、业务/风险双链路、Swap Set 四象限）远超单一规则挖掘任务的范畴。

#### 范围拆分

| 保留在本文档（规则挖掘附加功能） | 拆分至策略诊断 SOP 任务 |
|--------------------------------|------------------------|
| ✅ CSV 文件上传先验规则 | Swap Set 四象限分析 |
| ✅ 两种 CSV 格式（结构化 + 表达式） | 人数 + 金额双口径影响分析 |
| ✅ 上传后即时校验（列名匹配） | 换入/换出客群画像 |
| ✅ 简单阈值对比表（同特征新旧值） | 业务影响链（通过率→放款量→收入） |
| ✅ 保留现有 textarea 输入（向后兼容） | 风险影响链（坏账率→损失额→资产质量） |
| | 服务于规则挖掘 + 评分卡（跨任务） |

> 详细方案见 [`strategy_diagnosis_plan.md`](./strategy_diagnosis_plan.md)

#### 同步补全项

| 项 | 说明 |
|---|------|
| AI Prompt 金额分析引导 | 现有 `AmountAnalyzer` 已全栈实现但 `AI_analysis_prompts.py` 未引导解读金额指标，登记到 Phase 1.7 批次 2 |

---

### 📌 快速回顾（开发前必读）

**作用与目标**：增强规则挖掘任务的"先验规则"输入能力。当前用户只能在 textarea 中手写 Python 表达式（如 `(age > 30)`），本方案支持**CSV 文件上传 + 即时校验 + 简单阈值对比**。
