...
| 15 | Linux 部署脚本 | `scripts/deploy_linux.sh` + `scripts/service.sh` | ✅ 完成 |
| 16 | 模型配置同步脚本 | `scripts/sync_model_configs.sh` | ✅ 完成 |
...
| 4 | CVM `docker-compose.yml` 中硬编码了加密密钥 | 安全风险（仅 CVM 本地文件） | P2 | 后续应改为通过 `.env` 文件或 Docker secrets 管理 |
