...
### 20. CVM部署测试发现的已知限制/技术债（2026-07-03排查发现；四种部署方式全部测试通过，限制①已修复）

> 背景：本轮 CVM（TencentOS/RPM系）重新部署测试**全部四种部署方式均已完成并通过**——测试1 Docker单用户模式、测试2 非Docker手动部署（多用户）、测试3 Docker离线部署、测试4 Docker在线多用户部署（作为CVM最终常驻状态，已迁移6个存量账户，长期运行）。详细测试记录见 `docs/cvm_deployment_test_plan.md` 第四轮。测试期间除已当场修复的代码/脚本 bug（`executor.py` PEP695语法兼容性、`deploy_manual.sh` 缺少 `DEV_MODE=false`、`deploy_manual.sh` Tailwind `--content`覆盖config导致CSS不完整、`prepare_offline.sh`误打包`demo/chat/.next`本地构建缓存~240MB、LLM Manager前端403无权限提示不友好、账户锁定计数器并发竞态导致误锁定）外，还发现1个不阻塞部署的已知限制（限制②），登记如下；限制①经用户确认后已于当日完成架构级修复。

| 项目 | 内容 |
|------|------|
| **限制①（✅ 2026-07-03已修复，分两步）：生产模式下 Basic Auth 退出登录无法真正切换账号** | **现象**：同源生产部署（`DEV_MODE=false`+`ENABLE_AUTH=true`）下，用户点"退出登录"后浏览器自动用**原生缓存**的 Basic Auth 凭证重新登录，无法切换账号；浏览器无痕窗口经实测也不可靠。**根因**：整页导航访问 `/`（及 `/llm-manager`）时若未认证，中间件保留 `WWW-Authenticate` 头触发浏览器原生弹窗，一旦登录成功浏览器会长期缓存该凭证，JS的 `clearAuth()` 清不掉这份浏览器原生缓存。**影响范围**：写在共享后端代码 `API/auth_middleware.py` 里，Docker多用户模式与非Docker手动多用户模式同等程度受影响（Docker单用户模式因 `ENABLE_AUTH=false` 不涉及）。<br>**修复第一步（commit `04b21ad`，事后验证不充分）**：把 `/`、`/llm-manager`、`/llm-manager/` 三个页面壳子路径纳入 `AUTH_WHITELIST_EXACT`，公开可加载，鉴权交给页面加载后的 AJAX 探测（`/auth/me`）+ 自定义登录框，相应把 `/llm-manager` 的 admin 角色前置检查下放到具体管理 API。**CVM实测用户浏览器仍复现问题**——排查发现：浏览器对 `Basic` 方案的原生缓存/自动重发是协议层行为，只要浏览器历史上（哪怕是本次修复上线前测试时）用 `Basic` 凭证认证成功过一次，之后会对该域名下**所有**请求（含 JS 未显式设置 `Authorization` 头的 AJAX 调用）自动注入缓存凭证——仅去掉页面壳子的401挑战头治不好已经被浏览器缓存过的旧凭证，无痕窗口也不可靠。<br>**✅ 修复第二步——真正根治（commit `870cf91`）**：认证方案从标准 `Basic` 改为自定义方案名 `CWAuth`（编码方式不变，仍是base64("user:pass")，只改方案前缀词），涉及 `API/auth_middleware.py`（判断前缀+移除全部`WWW-Authenticate`响应头）、`demo/chat/lib/config.ts`、`demo/chat/components/LoginDialog.tsx`、`llm_manager_integrated/frontend/shared/js/auth.js`。原理：浏览器只对它认识的方案（`Basic`/`Digest`/`NTLM`/`Negotiate`）做缓存+自动注入，自定义方案名只是不透明字符串，不会有任何特殊处理——即使浏览器仍留有旧`Basic`缓存并继续自动注入，后端也只认`CWAuth`前缀，旧凭证被直接判定无效，不需要用户清缓存/重启浏览器；第一步的"公开壳子+AJAX鉴权"架构保留，两步结合才是完整方案。**验证结论仍成立**：不会重新引入"首屏JS未加载完前怎么鉴权"的历史顾虑。<br>**验证结果**：`tests/test_auth_middleware.py`等77项回归测试全通过（同步更新4处断言：`/llm-manager/`不再是admin-only路由、`WWW-Authenticate`不再在任何场景发送、Base64解码测试改用`CWAuth`前缀）；CVM实测：`/`无认证200；用旧`Basic`方案头访问`/workspace/files`返回401（证明后端不再接受旧方案）；401响应确认不带`WWW-Authenticate`头；两个前端已重新构建/同步产物到CVM并重启生效。 |
| **限制②（低优先级）：`deploy_manual.sh` 系统依赖检测硬编码 `dpkg`，RPM系发行版下完全失效** | **现象**：`deploy_manual.sh` 的系统依赖检测逻辑硬编码调用 `dpkg -s` 判断系统库是否已安装，但在 RPM 系发行版（如本次CVM实测的 TencentOS，用 yum/rpm 而非 dpkg）上该命令根本不存在，导致检测环节**静默失效**——不报错、不提示"检测不到"，直接跳过，可能使真实缺失的系统库延迟到后续编译/运行阶段才暴露，报错信息也不够精确。<br>**发现于**：2026-07-03 CVM非Docker手动部署测试（测试2）。<br>**影响范围**：仅限非Docker手动部署路径（`deploy_manual.sh`）且仅限目标机器是 RPM 系发行版（CentOS/RHEL/TencentOS/Fedora等）；Docker部署路径不受影响（依赖已固化在镜像内，构建时用的是 Debian 系基础镜像的包管理器，与此无关）。<br>**优先级**：低（未阻塞本次CVM测试实际部署成功；不修复的直接后果仅是"库缺失时报错时机后移、提示不够精确"，不影响能否部署成功）。<br>**建议修复方向**：改为先探测 `/etc/os-release` 判断发行版系，再分支调用 `dpkg -s`（Debian系）或 `rpm -q`（RPM系）；或统一改用更通用的探测方式（如直接用 `python -c "import xxx"` 探测Python库依赖，不依赖系统包管理器命令）。**（唯一仍待排期项，其余全部已修复）** |
| **✅ 已修复：`prepare_offline.sh` 误打包 `demo/chat/.next`（Next.js本地构建缓存，约240MB）** | **现象**：测试3（离线部署）期间发现，`rsync`打包`source/`目录时排除列表遗漏了`.next`，导致每份离线包多带约240MB完全无用的数据（打包后压缩体积也多约57MB）。**根因**：离线部署走`Dockerfile`多阶段构建（`frontend-builder`阶段独立`npm run build`后只`COPY dist/`），`source/`目录本身也只提供运行时配置+bind mount，两者都不依赖宿主机这份`.next`缓存。**修复**：`scripts/prepare_offline.sh`新增`--exclude='.next'`（commit `911edc7`）。**验证**：重新完整跑一遍"生成离线包→隔离目录解压→`deploy_offline.sh`部署→功能验证"全流程，`demo`目录从248M降至5.6M，`.next`确认不存在，服务功能（首页/LLM Manager/CSS/认证边界）全部正常。 |
| **✅ 已修复：账户锁定计数器并发竞态，导致浏览器缓存的失效凭证被误判为多次"主动输错密码"提前触发锁定** | **现象**：测试3期间用`sylarswwang`账户登录，用户本人未做任何操作，账户却在9毫秒内被记录5次"登录失败"并触发15分钟锁定。**根因**：①`_failure_tracker`是普通dict，`authenticate()`经`run_in_threadpool`跑在多线程环境下，"读取-加一-写回"不是原子操作，存在竞态（日志中"剩余次数"从4跳到2又重复出现一次2，即为证据）；②浏览器`localStorage`缓存的一份早前会话遗留的失效凭证，被全局fetch拦截器自动附加到页面刷新时并发发起的多个后台请求（`workspace/tree`、`workspace/files`等），这些并发请求各自独立失败、各自独立计入失败次数，本应算"同一份错误凭证的一次失效"却被算成5次不同的登录尝试。**修复（`API/auth_middleware.py`+`API/user_admin_api.py`，commit `5199715`）**：①`_failure_tracker`读写全部加`threading.Lock`保证原子性；②`_record_failure`新增去重逻辑——同一份凭证（sha256哈希比较，不存明文）在2秒短时间窗口内的重复失败只计1次，不同密码的失败仍正常分别计数，不削弱真实暴力破解防护。**验证**：回归测试78项通过（新增专项去重测试）；CVM实测：5个并发请求用同一份错误凭证只记1次失败（不误锁定）；4个不同错误密码依次尝试正常累加到4次（暴力破解防护未受影响）。 |
| **✅ 已修复：LLM Manager 前端非admin用户遇到403无明确提示（一直显示"加载中"）** | **现象**：`/llm-manager`页面壳子公开后，非admin用户能打开页面，但调用渠道/日志/统计/系统信息等admin专属API会收到403，此前统一走`showError()`（浏览器原生`alert`），关闭弹窗后原容器停留在"加载中"骨架，容易被误认为是bug。**修复**：`llm_manager_integrated/frontend/scripts/main.js`四个数据加载函数新增403专门识别，直接替换成明确的"🔒无权限访问XXX"提示（commit `12b3cb7`）。 |
| **登记来源** | `docs/cvm_deployment_test_plan.md` 本轮重新部署测试第四轮，**四种部署方式（Docker单用户/非Docker手动多用户/Docker离线/Docker在线多用户常驻）全部测试通过**，测试期间发现并修复7个真实bug（详见上表），另登记限制②（低优先级dpkg探测）待排期 |
...
7. 2026-07-03 补充：CVM重新部署测试第四轮，**四种部署方式（Docker单用户/非Docker手动多用户/Docker离线/Docker在线多用户常驻）全部测试通过**（第20项）。测试期间发现并修复7个真实bug，含架构级修复：生产模式Basic Auth退出登录无法切换账号（认证方案从`Basic`改为自定义`CWAuth`，commit`870cf91`，从根源避免浏览器原生凭证缓存）、账户锁定计数器并发竞态+误锁定（加锁+短时间窗口同凭证去重，commit`5199715`）、离线包误打包`.next`缓存~240MB（commit`911edc7`）、LLM Manager 403无权限提示优化（commit`12b3cb7`）等；仅剩1项低优先级限制（`deploy_manual.sh`依赖检测硬编码dpkg，RPM系发行版失效）待排期
