...
|------|------|------|
| 上传文件 | AI 对话 | 代码执行输出 |
| 选择任务类型 | SOP 任务结果 | 阶段结果预览 |
| 切换专家模式 | 模型选择器（底部） | 结果可视化 |
| 历史任务 | | AI 分析建议 |

### 8.2 上传数据

左列顶部点击 📎 按钮或拖拽文件。支持 CSV（`.csv`）、Excel（`.xlsx`/`.xls`）。

### 8.3 选择模型

中列底部下拉菜单选择 LLM 渠道（管理员在 LLM Manager 中配置）。

### 8.4 规则挖掘任务

左列选择「规则挖掘」，中列对话区输入需求：

> 💬 "帮我做规则挖掘，目标变量是 is_default，最多选 5 条规则"
...
必填参数：`target_col`（好坏标签列名），其他有默认值可直接运行。

### 8.6 执行进度

各阶段依次执行，状态：🔵 进行中 → ✅ 完成 / ❌ 失败。右侧列实时显示当前阶段输出。

### 8.7 结果导出

任务完成后顶部工具栏可导出：Excel（数据表）、HTML（可视化图表）、Word（正式报告）。
