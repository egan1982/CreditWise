...
| **UI-3** | 附加分析 Tab 展示 | 任务完成后 → 点击"附加分析"Tab | 显示 AmountAnalysisPanel 组件：4 张汇总卡片（总金额/坏账金额/拦截比/累计召回率）+ 规则金额明细表 |
| **UI-4** | 汇总卡片数据正确性 | 查看 4 张卡片数值 | 总金额 > 0，坏账金额 > 0 且 < 总金额，拦截比 ∈ (0,1)，累计召回率 ∈ (0,1) |
