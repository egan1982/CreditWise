...
| **Phase 25** | **删除历史记录级联清理 + 批量删除** | ✅ 已完成 |

### Phase 25：删除历史记录级联清理（已完成）

> **优先级**: P3 | **预计工作量**: ~1天 | **创建日期**: 2026-04-15
...
> **创建日期**: 2026-04-16 | **来源**: 随记补充

**前端交互**（`TaskHistoryList.tsx` / `TaskHistoryCompact.tsx`）：
```
┌─────────────────────────────────────────────────────┐
│  ☐ 全选    已选 3 条    [🗑️ 批量删除]  [取消选择]   │
├─────────────────────────────────────────────────────┤
│  ☑ 规则挖掘 - 2026-04-15 16:25        ✅ 已完成    │
│  ☐ 评分卡开发 - 2026-04-15 14:30      ✅ 已完成    │
│  ☑ 规则挖掘 - 2026-04-14 10:00        ❌ 失败      │
│  ☑ 评分卡开发 - 2026-04-13 09:15      ✅ 已完成    │
└─────────────────────────────────────────────────────┘
```

- **多选模式**: 每行前加 checkbox，点击进入多选模式
- **全选**: 表头 checkbox 全选/取消全选（仅选中当前筛选结果）
- **批量删除确认弹窗**:
```
确认批量删除 3 条任务记录？
...
```python
# 新增批量删除端点
DELETE /sop/history/batch
Body: {
    "record_ids": ["id1", "id2", "id3"],
    "cleanup_files": true  # 默认 true
}
...
- 批量删除复用单条删除的级联清理逻辑
- 返回清理统计（删除数、失败数、释放空间）
- 运行中的任务不允许删除（前端 checkbox 禁用 + 后端校验）
