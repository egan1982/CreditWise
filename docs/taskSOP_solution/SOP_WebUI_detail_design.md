...
## 十、Chat 任务入口交互优化方案（✅ 已实施）

> **优先级**: P2 | **预计工作量**: ~1-2天 | **状态**: ✅ 已实施（2026-04-16） | **创建日期**: 2026-04-15  
> **开发评审**: 轻量 Design Review 已完成（2026-04-16）

#### 📋 Design Review 评审记录（2026-04-16）

> **审查方法**: 轻量 design-review（交互状态 + 用户旅程为主）

**评分总览**

| Pass | 维度 | 初始 | 修正后 | 说明 |
|------|------|:---:|:---:|------|
| 1 | 信息架构 | 8 | 8 | 卡片内容层级清晰，数据源（GET /sop/tasks/{task_id}）明确 |
| 2 | 交互状态 | 5 | 8 | 补充卡片三态 + API loading/error（见下方 D1） |
| 3 | 用户旅程 | 7 | 8 | 执行中跳过时应有文字提示而非静默（见下方 D2） |
| 4 | 设计系统 | 9 | 9 | 复用 shadcn/ui Card + Button |
| **总分** | | **7** | **8** | |
...
| # | 建议 | 说明 |
|---|------|------|
| **D1** | **卡片交互三态** | 轻量卡片应有 3 种持久状态：① **pending**（待确认）：两个按钮正常展示；② **confirmed**（已使用）：绿色边框 + "✅ 已进入任务配置" + 无按钮；③ **dismissed**（已跳过）：灰色半透明 + "已跳过" + 无按钮。API loading 时展示 Skeleton，API error 时用 task_type 字符串展示最小卡片 |
| **D2** | **执行中跳过需提示** | §10.6 "执行中跳过"时不应静默走 chat mode。应给文字提示（"检测到您本会话已有一个评分卡任务正在执行，是否要启动新的？"），避免用户困惑 |
| **D3** | **ConfigPanel 参数注入** | §10.5 第 2 点"注入 LLM 提取的 params 到 ConfigPanel formValues"——当前 ConfigPanel 无外部注入机制，需新增 `initialParams` prop 或通过 context 传入 |
...
1. 新建轻量确认卡片组件 `sop/TaskConfirmCard.tsx`（~230行）：展示任务名称 + 描述 + 阶段列表 + 预计时间 + [使用此任务]/[继续对话] 按钮；数据来自 `GET /sop/tasks/{task_id}`；含三态（pending/confirmed/dismissed）+ loading Skeleton + error 兜底
2. 用户点击"使用此任务" → 调用 `setSelectedTaskId` + `setShowConfigPanel(true)` + 注入 LLM 提取的 params 到 ConfigPanel formValues（通过 `initialParams` prop）
3. 用户点击"继续对话" → 切回 chat mode，LLM 以自然语言回答
4. 将 `isTaskParamJson()` 解析函数（~70行）抽离至 `lib/taskParamParser.ts`，**删除** `TaskParamCard.tsx`（~1372行）
5. 为 `TaskConfigPanel` 新增 `initialParams?: Record<string, any> | null` prop + 独立 useEffect 处理参数注入（覆盖同 taskId 二次打开场景）
