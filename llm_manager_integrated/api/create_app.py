...
def create_api_app(
    ...
    Examples:
        # 基本集成
        from llm_api_manager.api import create_api_app
        llm_app = create_api_app()
        main_app.mount("/api/llm-manager", llm_app)
        
        # 带配置的集成
        llm_app = create_api_app(
            config={
                "cors_origins": ["http://localhost:3000"],
                "app_title": "LLM Manager API"
            },
    ...
    from .routes.proxy import models_router
    app.include_router(models_router, prefix=f"{api_prefix}/proxy", tags=["模型列表"])
...
def mount_to_fastapi(
    ...
    Returns:
        LLM Manager API应用实例
    """
    llm_app = create_api_app(config=config, prefix=prefix)
